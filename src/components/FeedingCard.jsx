...
// Quantité bue saisie (0 inclus) — un champ vide "" signifie « non renseigné »
const isBuSaisi = v => v !== undefined && v !== null && String(v).trim() !== "" && !isNaN(parseFloat(v));
  ...
  return (
    <div style={{ background: cardBg, borderRadius: 16, padding: "16px", marginBottom: 12, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", borderLeft: "4px solid #e8906a" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: f.note ? 10 : 0 }}>
        ...
        {isBuSaisi(f.complMaternelBu) && <Tag color="#c8e6c9" dark={dark}>🤱 {f.complMaternelBu} ml bu</Tag>}
        {isBuSaisi(f.complCommercialBu) && <Tag color="#c8e6c9" dark={dark}>🏭 {f.complCommercialBu} ml bu</Tag>}
        {/* Legacy fallback */}
        ...
        {!seinTag && !(parseFloat(f.complMaternel) || 0) && !(parseFloat(f.complCommercial) || 0) && !isBuSaisi(f.complMaternelBu) && !isBuSaisi(f.complCommercialBu) && !f.complement && !f.pipi && !f.caca && !f.vitamineD && !f.nombril && !f.yeux && !(parseInt(f.regurgi) || 0) && <span style={{ fontSize: 12, color: "#ccc" }}>Aucun détail</span>}
      </div>
