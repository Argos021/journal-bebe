...
function GrowthLineChart({ data, valueKey, color, unit, dark }) {
  ...
  return (
    <div style={{overflowX:"auto", position:"relative"}}>
      <svg width={W} height={H} style={{display:"block",margin:"0 auto"}}
        ...
        {sorted.map((d,i) => {
          ...
          return (
            <g key={i} onClick={e => {
              e.stopPropagation();
              setTooltip(tooltip?.i === i ? null : { x: px(i), y: py(val), d, i, val, prevVal });
            }} style={{cursor:"pointer"}}>
              {/* Larger tap target */}
              <circle cx={px(i)} cy={py(val)} r={12} fill="transparent"/>
              <circle cx={px(i)} cy={py(val)} r={isSelected ? 6 : 4}
                fill={isSelected ? "white" : color}
                stroke={isSelected ? color : (dark?"#1a1a2e":"white")}
                strokeWidth={isSelected ? 3 : 2}/>
              <text x={px(i)} y={H-4} textAnchor="middle" fontSize={9} fill={isSelected ? color : textCol}>
      ...
      {/* Tooltip bubble */}
      {tooltip && (() => {
        const val = tooltip.val;
        const prevVal = tooltip.prevVal;
        const diff = prevVal !== null ? (val - prevVal) : null;
        const diffPct = prevVal !== null ? ((val - prevVal) / prevVal * 100) : null;
        const birthPct = birthVal && !tooltip.d._isBirth ? ((val - birthVal) / birthVal * 100) : null;
        const label = tooltip.d._label || formatDateShort(tooltip.d.date);

        // Position bubble: flip left if near right edge
        const svgLeft = 0;
        const bubbleW = 160;
        const rawX = tooltip.x - bubbleW / 2;
        const clampedX = Math.max(0, Math.min(rawX, W - bubbleW));
        ...
        return (
          <div style={{
            ...
            background: dark?"#2a2a3e":"white",
            border:`2px solid ${color}`,
            borderRadius:12,
            padding:"10px 14px",
            fontSize:12,
            color: dark?"#f5deb3":"#333",
            ...
            <div style={{fontSize:14,fontWeight:"bold",marginBottom:4}}>
              {val.toFixed(unit==="g"?0:1)} {unit}
            </div>
            {diff !== null && (
              <div style={{color: diff>=0?"#4caf50":"#e53935", marginBottom:2}}>
                {diff>=0?"▲":"▼"} {Math.abs(diff).toFixed(unit==="g"?0:1)} {unit} vs précédent
              </div>
            )}
            {diffPct !== null && (
              <div style={{color: diff>=0?"#4caf50":"#e53935", marginBottom:2}}>
            ...
            {birthPct !== null && (
              <div style={{color: birthPct>=0?"#1565c0":"#ff7043", fontSize:11, marginTop:4, borderTop:`1px solid ${dark?"#333":"#eee"}`, paddingTop:4}}>
                {birthPct>=0?"▲":"▼"} {Math.abs(birthPct).toFixed(1)}% vs naissance
              </div>
            )}
          </div>
        );
