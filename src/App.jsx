...
function TireLaitTab({ tireLait, dark, cardBg, textPrimary, textSecondary, dynInputStyle, borderColor, setConfirmDelete, showToast, onFormOpen, onFormClose }) {
  ...
  const graphDataJour = (grouped[graphDay]||[])
    .map(e=>({label:e.time,val:parseFloat(e.quantite)||0}))
    .sort((a,b)=>a.label.localeCompare(b.label));

  const graphData = graphMode==="mois"?graphDataMois:graphDataJour;
      ...
      <div style={{background:cardBg,borderRadius:14,padding:14,marginBottom:14,boxShadow:"0 2px 12px rgba(0,0,0,0.07)"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,flexWrap:"wrap"}}>
          <div style={{display:"flex",borderRadius:20,border:`1.5px solid ${dark?"#3a3a5e":"#e8c5a8"}`,overflow:"hidden"}}>
            {[["mois","📆 Mois"],["jour","📅 Jour"]].map(([m,l],i)=>(
              <button key={m} onClick={()=>setGraphMode(m)} style={{padding:"5px 12px",border:"none",borderLeft:i>0?`1px solid ${dark?"#3a3a5e":"#e8c5a8"}`:"none",background:graphMode===m?(dark?"rgba(66,165,245,0.25)":"#e3f2fd"):"transparent",color:graphMode===m?"#1565c0":textSecondary,fontWeight:graphMode===m?"bold":"normal",fontSize:12,cursor:"pointer"}}>{l}</button>
            ))}
          </div>
          {graphMode==="jour" && (
            <select value={graphDay} onChange={e=>setGraphDay(e.target.value)} style={{flex:1,padding:"5px 8px",borderRadius:10,border:`1px solid ${dark?"#444":"#ddd"}`,background:dark?"#1e1e30":"white",color:textPrimary,fontSize:12,fontFamily:"Georgia,serif",outline:"none"}}>
              {daysInMonth.filter(d=>grouped[d]).map(d=>(
                <option key={d} value={d}>{formatDate(d)}</option>
              ))}
              {!daysInMonth.filter(d=>grouped[d]).length && <option value="">Aucune session</option>}
            </select>
          )}
        </div>
        {graphData.length>0 ? (
          <svg width={W} height={H} style={{display:"block",margin:"0 auto",overflow:"visible"}}>
            ...
            {graphData.map((d,i)=>(
              <g key={i}>
                ...
                {d.val>0&&<text x={pxBar(i,graphData.length)+barW(graphData.length)/2} y={py(d.val)-3} textAnchor="middle" fontSize={8} fill={dark?"#90caf9":"#1565c0"} fontWeight="bold">{d.val}</text>}
              </g>
            ))}
          </svg>
        ) : (
          <div style={{textAlign:"center",color:"#aaa",fontSize:12,padding:"20px 0"}}>
            {graphMode==="jour"?"Aucune session ce jour — sélectionne un autre jour":"Aucune donnée pour ce mois"}
          </div>
        )}
      </div>
