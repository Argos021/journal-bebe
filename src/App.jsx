                        ...
                        {jData.map((d,i)=>{
                          ...
                          return (
                            <g key={i}>
                              ...
                              {d.val>0&&<text x={x+w/2} y={pY2(d.val)-3} textAnchor="middle" fontSize={8} fill={dark?"#f48fb1":"#c2185b"} fontWeight="bold">{fmtVal2(Math.round(d.val))}</text>}
                            </g>
                          );
                        })}
                        </>);
          ...
          <svg width={W} height={rotateLabels?H+20:H} style={{display:"block",margin:"0 auto",overflow:"visible"}}>
            ...
            {graphData.map((d,i)=>(
              <g key={i}>
                ...
                {d.val>0&&<text x={pxBar(i,graphData.length)+barW(graphData.length)/2} y={pyR(d.val)-3} textAnchor="middle" fontSize={8} fill={dark?"#90caf9":"#1565c0"} fontWeight="bold">{fmtV(Math.round(d.val))}</text>}
              </g>
            ))}
          </svg>
          );
