    ...
    <div style={{padding:"16px",maxWidth:480,margin:"0 auto",paddingBottom:100}}>
      ...
      <div style={{background:cardBg,borderRadius:14,padding:14,marginBottom:14,boxShadow:"0 2px 12px rgba(0,0,0,0.07)"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,flexWrap:"wrap"}}>
          ...
          {graphMode==="jour" && (
            <select value={validGraphDay} onChange={e=>setGraphDay(e.target.value)} style={{flex:1,padding:"5px 8px",borderRadius:10,border:`1px solid ${dark?"#444":"#ddd"}`,background:dark?"#1e1e30":"white",color:textPrimary,fontSize:12,fontFamily:"Georgia,serif",outline:"none"}}>
              {daysInMonth.filter(d=>grouped[d]).map(d=>(
                <option key={d} value={d}>{formatDate(d)}</option>
              ))}
              {!daysInMonth.filter(d=>grouped[d]).length && <option value={selectedMonth+"-01"}>Aucune session ce mois</option>}
            </select>
          )}
        </div>
        {graphData.length>0 ? (
