    ...
    <div style={{ minHeight:"100vh", background:bg, fontFamily:"'Georgia',serif", padding:"0 0 120px", transition:"background 0.3s" }}>
        ...
        <div style={{display:"flex", borderTop:`1px solid ${dark?"rgba(255,255,255,0.1)":"rgba(0,0,0,0.08)"}`, marginTop:4}}>
          {[["profil","👶"],["journal","📓"],["croissance","📏"],["sante","🏥"]].map(([key,icon])=>{
            ...
            const urgentAppts = appointments.filter(a => {
              ...
              const diff = daysUntil(a.date, a.time);
              return diff >= 0 && diff <= 3;
            }).length;
            return (
              <button key={key} onClick={()=>setTab(key)} style={{flex:1,padding:"8px 0 6px",border:"none",background:"transparent",cursor:"pointer",borderBottom:tab===key?`3px solid ${dark?"#f48fb1":"#e8906a"}`:"3px solid transparent",transition:"all 0.15s",position:"relative"}}>
      ...
      {tab==="profil" && (
        <div style={{padding:"16px",maxWidth:480,margin:"0 auto"}}>
          <h2 style={{margin:"0 0 16px",fontSize:18,color:textPrimary}}>👶 Profil de bébé</h2>
          ...
          <SectionCard dark={dark} cardBg={cardBg}>
            ...
            {profile.dateNaissance && (() => {
              const timeStr = profile.heureNaissance || "00:00";
              const birth = new Date(profile.dateNaissance + "T" + timeStr + ":00");
              const now = new Date();
              ...
              return (
                <div style={{background:dark?"#1a2a3a":"#e3f2fd",borderRadius:10,padding:"8px 12px",marginTop:0,marginBottom:14,fontSize:13,color:dark?"#90caf9":"#1565c0",fontWeight:"bold",display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                  🎉 Âge : {ageStr}
                  <span style={{fontSize:11,fontWeight:"normal",color:dark?"#5a8aaa":"#5a8aaa"}}>({diffDays} jours)</span>
                  {profile.heureNaissance && <span style={{fontSize:11,fontWeight:"normal",color:dark?"#5a8aaa":"#5a8aaa"}}>· né à {profile.heureNaissance}</span>}
                </div>
              );
            })()}
          ...
          <SectionCard dark={dark} cardBg={cardBg}>
            ...
            {(profile.poidsNaissance || profile.tailleNaissance) && (
              <div style={{background:dark?"#1a1a2e":"#fdf6f0",borderRadius:10,padding:"10px 12px",marginTop:-4,fontSize:12,color:textSecondary}}>
                💡 Ces données apparaîtront comme point de départ dans l'onglet Croissance.
              </div>
            )}
          </SectionCard>

          <div style={{textAlign:"center",marginTop:4,fontSize:12,color:dark?"#555":"#bbb"}}>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:100,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
          <div style={{background:cardBg,borderRadius:"24px 24px 0 0",padding:"28px 20px 40px",width:"100%",maxWidth:480,boxShadow:"0 -8px 40px rgba(0,0,0,0.2)"}}>
            ...
            <div style={{display:"flex",gap:10,marginTop:8}}>
              <button onClick={()=>setShowCoucheForm(false)} style={btnSecondaryBase}>Annuler</button>
...
function GrowthLineChart({ data, valueKey, color, unit, dark }) {
  ...
  const W = 320, H = 140, padL = 44, padR = 12, padT = 16, padB = 28;
  ...
  return (
    <div style={{overflowX:"auto"}}>
      <svg width={W} height={H} style={{display:"block",margin:"0 auto"}}>
        ...
        {sorted.map((d,i) => (
          <g key={i}>
            <circle cx={px(i)} cy={py(parseFloat(d[valueKey]))} r={4} fill={color} stroke={dark?"#1a1a2e":"white"} strokeWidth={2}/>
            <text x={px(i)} y={H-4} textAnchor="middle" fontSize={9} fill={textCol}>
    ...
    <div style={{padding:"16px", maxWidth:480, margin:"0 auto"}}>
      ...
      {latest && (
        <div style={{background: dark?"#2a2a3e":"#fff8f0", borderRadius:16, padding:14, marginBottom:14, boxShadow:"0 2px 12px rgba(0,0,0,0.07)"}}>
          <div style={{fontSize:12, color:textSecondary, marginBottom:8, fontWeight:"bold"}}>Dernière mesure — {formatDateShort(latest.date)}</div>
          <div style={{display:"flex", gap:10}}>
            {latest.poids && (
              <div style={{flex:1, background: dark?"#1a3020":"#e8f5e9", borderRadius:12, padding:"10px 14px", textAlign:"center"}}>
                <div style={{fontSize:20, fontWeight:"bold", color: dark?"#a5d6a7":"#2e7d32"}}>
                  {toDisplayWeight(latest.poids)}<span style={{fontSize:11}}> {weightUnitLabel()}</span>
                </div>
            ...
            {latest.taille && (
              <div style={{flex:1, background: dark?"#1a2a40":"#e3f2fd", borderRadius:12, padding:"10px 14px", textAlign:"center"}}>
                <div style={{fontSize:20, fontWeight:"bold", color: dark?"#90caf9":"#1565c0"}}>
                  {toDisplayHeight(latest.taille)}
                </div>
                <div style={{fontSize:11, color:"#888"}}>
            ...
            {latest.crane && (
              <div style={{flex:1, background: dark?"#2a1a3a":"#f3e5f5", borderRadius:12, padding:"10px 14px", textAlign:"center"}}>
                <div style={{fontSize:20, fontWeight:"bold", color: dark?"#ce93d8":"#6a1b9a"}}>
                  {parseFloat(latest.crane).toFixed(1)}<span style={{fontSize:11}}> cm</span>
                </div>
                <div style={{fontSize:11, color:"#888"}}>
                  Crâne{craneChangeCm ? ` (+${craneChangeCm}cm)` : ""}
                </div>
              </div>
            )}
          </div>
      ...
      {view === "graphique" && growth.length > 0 && (
        <div style={{background:cardBg, borderRadius:16, padding:16, boxShadow:"0 2px 12px rgba(0,0,0,0.07)"}}>
          ...
          <GrowthLineChart
            ...
            dark={dark}
          />
          {sorted.filter(d => metric==="poids" ? d._poidsDisplay : d._tailleDisplay).length < 2 && (
...
function apptUrgency(dateStr, timeStr, done) {
  if (done) return "done";
  const d = daysUntil(dateStr, timeStr);
  if (d < 0) return "past";
  if (d < 1) return "today";   // within next 24h (same day, not yet passed)
  if (d <= 3) return "soon";
  return "future";
}

function generateICS(appt, babyName) {
  const dt = new Date(appt.date + "T" + (appt.time || "09:00") + ":00");
  const pad = n => String(n).padStart(2,"0");
  const fmt = d => `${d.getFullYear()}${pad(d.getMonth()+1)}${pad(d.getDate())}T${pad(d.getHours())}${pad(d.getMinutes())}00`;
  const end = new Date(dt.getTime() + 60*60*1000);
  const type = APPT_TYPES.find(t=>t.value===appt.type)?.label || "Rendez-vous";
  const ics = [
    "BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//BabyTracker//FR",
    "BEGIN:VEVENT",
    `DTSTART:${fmt(dt)}`,`DTEND:${fmt(end)}`,
    `SUMMARY:${type} — ${babyName}`,
    `DESCRIPTION:${appt.notes||""}`,
    `LOCATION:${appt.lieu||""}`,
    `UID:${appt.id}@babytracker`,
...
function SanteTab({ appointments, setAppointments, dark, cardBg, textPrimary, textSecondary, dynCardStyle, babyName, showToast, dynInputStyle, setConfirmDelete }) {
  ...
  function downloadICS(appt) {
    ...
    a.href = uri; a.download = `rdv_${appt.date}.ics`;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    showToast("📅 Fichier .ics téléchargé — ouvre-le pour ajouter au calendrier !");
  }
    ...
    <div style={{padding:"16px", maxWidth:480, margin:"0 auto"}}>
      ...
      {alerts.length > 0 && (
        <div style={{background:dark?"#2a2010":"#fff8e1",border:"2px solid #ffc107",borderRadius:12,padding:"10px 14px",marginBottom:14}}>
          ...
          {alerts.map(a=>{
            const d=daysUntil(a.date, a.time);
            const dDays=Math.floor(d);
            const type=APPT_TYPES.find(t=>t.value===a.type);
            const timing = d < 1 ? "Aujourd'hui" : dDays===1 ? "Demain" : `Dans ${dDays} jours`;
            return (
              <div key={a.id} style={{fontSize:12,color:dark?"#ffe082":"#5d4037",marginBottom:4,display:"flex",alignItems:"center",gap:6}}>
                <span style={{fontWeight:"bold",color:d<1?"#4caf50":dDays===1?"#ff9800":"#f57f17"}}>{timing}</span>
      ...
      {filtered.map(a=>{
        ...
        const type = APPT_TYPES.find(t=>t.value===a.type);
        const d = daysUntil(a.date, a.time);
        const dDays = Math.floor(Math.abs(d));
        const timingLabel = d < 0
          ? `Il y a ${dDays === 0 ? "moins d'1j" : dDays+"j"}`
          : d < 1 ? "Aujourd'hui"
          : dDays === 1 ? "Demain"
          : `Dans ${dDays}j`;
        const timingColor = d < 0 ? "#ff7043" : d < 1 ? "#4caf50" : d <= 3 ? "#ff9800" : textSecondary;
        return (
      ...
      {tab==="journal" && (
        <div style={{
          ...
          <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
            <span style={{background:dark?"rgba(0,0,0,0.75)":"rgba(255,255,255,0.95)",color:dark?"#90caf9":"#1565c0",fontSize:"12px",fontWeight:"bold",padding:"4px 10px",borderRadius:"20px",boxShadow:"0 2px 8px rgba(0,0,0,0.2)"}}>Couche</span>
            <button onClick={()=>{setCoucheForm({date:todayStr(),time:getNow(),pipi:false,caca:false,note:""});setShowCoucheForm(true);}}
              style={{width:"50px",height:"50px",borderRadius:"50%",border:"none",background:"linear-gradient(135deg,#64b5f6,#1565c0)",color:"white",fontSize:"22px",cursor:"pointer",boxShadow:"0 4px 16px rgba(21,101,192,0.5)",display:"flex",alignItems:"center",justifyContent:"center"}}>
              🧷
            </button>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
            <span style={{background:dark?"rgba(0,0,0,0.75)":"rgba(255,255,255,0.95)",color:dark?"#f48fb1":"#c2185b",fontSize:"12px",fontWeight:"bold",padding:"4px 10px",borderRadius:"20px",boxShadow:"0 2px 8px rgba(0,0,0,0.2)"}}>Boire</span>
            <button onClick={()=>{setForm({...initialForm,date:todayStr(),time:getNow()});setShowForm(true);setEditId(null);}}
              style={{width:"56px",height:"56px",borderRadius:"50%",border:"none",background:"linear-gradient(135deg,#e8906a,#e06b8a)",color:"white",fontSize:"28px",fontWeight:"bold",cursor:"pointer",boxShadow:"0 4px 20px rgba(232,144,106,0.55)",display:"flex",alignItems:"center",justifyContent:"center"}}>
              +
            </button>
          </div>
        </div>
