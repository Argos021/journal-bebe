                  ...
                  {jData.length>0 ? (
                    <svg width={W2} height={journalGraphMode==="mois"||journalGraphMode==="annee"?H2+20:H2} style={{display:"block",margin:"0 auto",overflow:"visible"}}>
                      {(() => {
                        ...
                        {jData.map((d,i)=>{
                          const x=bX(i,jData.length),w=bW(jData.length),h=Math.max(1,gH2eff-(pY2(d.val)-pT));
                          ...
                          return (
                            <g key={i}>
                              ...
                              {rotL ? (
                                <text x={x+w/2} y={labelY} textAnchor="end" fontSize={8} fill={tCol}
                                  transform={`rotate(-90,${x+w/2},${labelY})`}>{d.label}</text>
                              ) : (
                                <text x={x+w/2} y={H2-4} textAnchor="middle" fontSize={8} fill={tCol}>{d.label}</text>
                              )}
                              {d.val>0&&(rotL ? (
                                <text x={x+w/2} y={pY2(d.val)-3} textAnchor="middle" fontSize={8} fill={dark?"#f48fb1":"#c2185b"} fontWeight="bold"
                                  transform={`rotate(-90,${x+w/2},${pY2(d.val)-3})`}>
                                  {fmtVal2(Math.round(d.val))}
                                </text>
                              ) : (
                                <text x={x+w/2} y={pY2(d.val)-3} textAnchor="middle" fontSize={8} fill={dark?"#f48fb1":"#c2185b"} fontWeight="bold">{fmtVal2(Math.round(d.val))}</text>
                              ))}
                            </g>
                          );
                        })}
          ...
          <svg width={W} height={rotateLabels?H+20:H} style={{display:"block",margin:"0 auto",overflow:"visible"}}>
            ...
            {graphData.map((d,i)=>(
              <g key={i}>
                ...
                {rotateLabels ? (
                  <text
                    x={pxBar(i,graphData.length)+barW(graphData.length)/2}
                    y={padT+gHeff+6}
                    textAnchor="end"
                    fontSize={8}
                    fill={textCol}
                    transform={`rotate(-90,${pxBar(i,graphData.length)+barW(graphData.length)/2},${padT+gHeff+6})`}>
                    {d.label}
                  </text>
                ) : (
                ...
                {d.val>0&&(rotateLabels ? (
                  <text x={pxBar(i,graphData.length)+barW(graphData.length)/2} y={pyR(d.val)-3} textAnchor="middle" fontSize={8} fill={dark?"#90caf9":"#1565c0"} fontWeight="bold"
                    transform={`rotate(-90,${pxBar(i,graphData.length)+barW(graphData.length)/2},${pyR(d.val)-3})`}>
                    {fmtV(Math.round(d.val))}
                  </text>
                ) : (
                  <text x={pxBar(i,graphData.length)+barW(graphData.length)/2} y={pyR(d.val)-3} textAnchor="middle" fontSize={8} fill={dark?"#90caf9":"#1565c0"} fontWeight="bold">{fmtV(Math.round(d.val))}</text>
                ))}
              </g>
            ))}
          </svg>
