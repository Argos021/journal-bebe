...
function GrowthLineChart({ data, valueKey, color, unit, dark }) {
  ...
  return (
    <div style={{overflowX:"auto", position:"relative"}}>
        ...
        {sorted.map((d,i) => {
          ...
          return (
            <g key={i} onClick={e => handleDotClick(e, d, i, val, prevVal)} style={{cursor:"pointer"}}>
              <circle cx={px(i)} cy={py(val)} r={14} fill="transparent"/>
      ...
      {/* Tooltip via portal — renders directly in body, never clipped */}
      {tooltip && createPortal((() => {
        const { val, prevVal, screenX, screenY } = tooltip;
        ...
        // Position above the dot, centered, clamped to viewport
        const left = Math.max(8, Math.min(screenX - bubbleW/2, window.innerWidth - bubbleW - 8));
        const top = screenY - 16; // slightly above the dot
