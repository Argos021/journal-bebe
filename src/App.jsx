            ...
            <div style={{padding:"12px 16px 0",maxWidth:480,margin:"0 auto"}}>
              <div style={{background:cardBg,borderRadius:14,padding:14,boxShadow:"0 2px 12px rgba(0,0,0,0.07)"}}>
                {(() => {
                  ...
                  function Row({s, accent, label}) { return (
                  ...
                  return (
                    <div>
                      {/* Month section with date filter */}
                      <div style={{marginBottom:10}}>
                        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
                          <div style={{fontSize:11,fontWeight:"bold",color:dark?"#f48fb1":"#e06b8a"}}>
                            📅 {monthLabel(selectedMonth)}{hasFilter?" (filtré)":""}
                          </div>
                          <button
                            onClick={()=>setStatsDateFilter(f=>f.start||f.end?{start:"",end:""}:{start:`${selectedMonth}-01`,end:`${selectedMonth}-${String(new Date(selY,selM,0).getDate()).padStart(2,"0")}`})}
                            style={{fontSize:10,padding:"2px 8px",borderRadius:10,border:`1px solid ${hasFilter?"#e06b8a":(dark?"#444":"#ddd")}`,background:hasFilter?(dark?"rgba(224,107,138,0.15)":"#fce4ec"):(dark?"#2a2a3e":"#f5f5f5"),color:hasFilter?"#e06b8a":(dark?"#aaa":"#888"),cursor:"pointer",fontWeight:"bold"}}>
                            🗓️ {hasFilter?"Effacer filtre":"Filtrer dates"}
                          </button>
                        </div>
                        {hasFilter && (
                          <div style={{display:"flex",gap:8,marginBottom:8,alignItems:"center"}}>
                            <input type="date"
                              value={statsDateFilter.start}
                              min={`${selectedMonth}-01`}
                              max={statsDateFilter.end||`${selectedMonth}-${String(new Date(selY,selM,0).getDate()).padStart(2,"0")}`}
                              onChange={e=>setStatsDateFilter(f=>({...f,start:e.target.value}))}
                              style={{flex:1,padding:"6px 10px",borderRadius:10,fontSize:12,boxSizing:"border-box",outline:"none",fontFamily:"Georgia,serif",border:`1px solid ${dark?"#444":"#ddd"}`,background:dark?"#1e1e30":"white",color:textPrimary}}/>
                            <span style={{color:textSecondary,fontSize:12}}>→</span>
                            <input type="date"
                              value={statsDateFilter.end}
                              min={statsDateFilter.start||`${selectedMonth}-01`}
                              max={`${selectedMonth}-${String(new Date(selY,selM,0).getDate()).padStart(2,"0")}`}
                              onChange={e=>setStatsDateFilter(f=>({...f,end:e.target.value}))}
                              style={{flex:1,padding:"6px 10px",borderRadius:10,fontSize:12,boxSizing:"border-box",outline:"none",fontFamily:"Georgia,serif",border:`1px solid ${dark?"#444":"#ddd"}`,background:dark?"#1e1e30":"white",color:textPrimary}}/>
                          </div>
                        )}
                        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                          <span style={{background:dark?"#3a2a10":"#fdebd0",borderRadius:20,padding:"3px 10px",fontSize:12,color:textPrimary,fontWeight:"bold"}}>🍼 {ms.total} · {ms.avg}/j</span>
                          <span style={{background:dark?"#1a3050":"#ddeeff",borderRadius:20,padding:"3px 10px",fontSize:12,color:dark?"#90caf9":"#1565c0",fontWeight:"bold"}}>💧 {ms.pipi} · {ms.avgPipi}/j</span>
                          <span style={{background:dark?"#3a2a10":"#fdebd0",borderRadius:20,padding:"3px 10px",fontSize:12,color:dark?"#ffcc80":"#e65100",fontWeight:"bold"}}>💩 {ms.caca} · {ms.avgCaca}/j</span>
                          {ms.cMat>0 && <span style={{background:dark?"#3a1a2a":"#f9c6d8",borderRadius:20,padding:"3px 10px",fontSize:12,color:dark?"#f48fb1":"#c2185b",fontWeight:"bold"}}>🤱 {ms.cMat.toFixed(0)}ml · {ms.avgMat}ml/j</span>}
                          {ms.cComm>0 && <span style={{background:dark?"#1a2a3a":"#e3f2fd",borderRadius:20,padding:"3px 10px",fontSize:12,color:dark?"#90caf9":"#1565c0",fontWeight:"bold"}}>🏭 {ms.cComm.toFixed(0)}ml · {ms.avgComm}ml/j</span>}
                          {ms.cTotal>0 && <span style={{background:dark?"#2a2a2a":"#f5f5f5",borderRadius:20,padding:"3px 10px",fontSize:12,color:textSecondary,fontWeight:"bold"}}>🥛 {ms.cTotal.toFixed(0)}ml total · {ms.avgTotal}ml/j</span>}
                        </div>
                      </div>

                      <div style={{borderTop:`1px solid ${dark?"rgba(255,255,255,0.08)":"rgba(0,0,0,0.06)"}`,marginBottom:10}}/>

                      {/* All-time section — never affected by date filter */}
