...
function GrowthLineChart({ data, valueKey, color, unit, dark, whoTable, birthDate, weightUnit }) {
  ...
  // Zoom state: viewStart/viewEnd = fraction of data range visible (0 to 1)
  ...
  function applyZoom(centerFrac, scaleFactor) {
    setViewStart(s => {
      setViewEnd(e => {
        const span = e - s;
        const newSpan = clamp(span / scaleFactor, 0.05, 1);
        const newStart = clamp(centerFrac - (centerFrac - s) * (newSpan / span), 0, 1 - newSpan);
        const newEnd   = clamp(newStart + newSpan, newSpan, 1);
  ...
  // Cleaner zoom using direct state
  function zoomAt(centerFrac, scaleFactor) {
    setViewStart(prevS => {
      const prevE = prevS + (1 - prevS); // will be overridden below
      return prevS; // placeholder
    });
    // Use a ref-based approach instead
    viewRef.current = { s: viewStart, e: viewEnd };
    const { s, e } = viewRef.current;
    const span = e - s;
    const newSpan = clamp(span / scaleFactor, 0.05, 1);
    const ratio = (centerFrac - s) / span;
    const newStart = clamp(centerFrac - ratio * newSpan, 0, 1 - newSpan);
  ...
  function handleWheel(e) {
    ...
    const span = ve - s;
    const newSpan = clamp(span / scaleFactor, 0.05, 1);
    const newStart = clamp(s + relX * (span - newSpan), 0, 1 - newSpan);
    setViewStart(newStart);
  ...
  function handleTouchStart(e) {
    if (e.touches.length === 2) {
      ...
      lastTouchMid.current = rect ? (midX - rect.left) / rect.width : 0.5;
    }
  }
  ...
  function handleTouchMove(e) {
    if (e.touches.length === 2 && lastTouchDist.current) {
      ...
      const center = lastTouchMid.current ?? 0.5;
      const { s, e: ve } = viewRef.current;
      const span = ve - s;
      const newSpan = clamp(span / scaleFactor, 0.05, 1);
      ...
      setViewStart(newStart);
      setViewEnd(newStart + newSpan);
      viewRef.current = { s: newStart, e: newStart + newSpan };
    }
  }
  ...
  function resetZoom() { setViewStart(0); setViewEnd(1); }
  ...
  const dataPoints = sorted.map((d,i) => {
    ...
    const cx = pxVal(xVal);
    return { d, i, val, prevVal, cx, xVal };
  });
    ...
    <div style={{position:"relative"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4,padding:"0 4px"}}>
        <span style={{fontSize:10,color:dark?"#555":"#bbb"}}>
          {isZoomed ? `Zoom: ${(1/(viewEnd-viewStart)).toFixed(1)}x` : "🔍 Pinch ou molette"}
        </span>
        {isZoomed && (
        ...
        <svg ref={svgRef} width={W} height={H} style={{display:"block",margin:"0 auto",overflow:"hidden"}}
          ...
          <defs>
            <clipPath id="chartClip">
              <rect x={padL} y={padT} width={gW} height={gH+padB}/>
            </clipPath>
          </defs>
          ...
          {/* X labels — outside clip so they don't get cut */}
          {dataPoints.map(({d,i,cx})=>{
            if (cx < padL - 5 || cx > W - padR + 5) return null;
            return (
              <text key={i} x={cx} y={H-4} textAnchor="middle" fontSize={9}
                fill={tooltip?.i===i?color:textCol}>
