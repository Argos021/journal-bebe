...
export default function BabyTracker({ onRegisterShowBoire, onRegisterShowCouche, onTabChange, onFormOpen, onFormClose }) {
  ...
  useEffect(() => {
    ...
    unsubs.push(onSnapshot(collection(db, "tireLait"), snap => {
      setTireLait(snap.docs.map(d => ({id: d.id, ...d.data()})));
    }));
  ...
  function handleEdit(f) {
    ...
    setForm({date:f.date||todayStr(),time:f.time,durationH:dH,durationM:dM,seinPremier,
      ...
      pipi:f.pipi,caca:f.caca,vitamineD:f.vitamineD||false,nombril:f.nombril||false,yeux:f.yeux||false,regurgi:f.regurgi||0,note:f.note});
        ...
        <div>
          ...
          {(() => {
            ...
            const jValidDay = (() => {
              ...
              const days = jDaysInMonth.filter(d=>jGrouped[d]);
              return days.length>0?days[days.length-1]:selectedMonth+"-01";
            })();
            const fmtVal = v => v>=1000 ? (v/1000).toFixed(2)+"L" : v+"ml";
            const getMl = f => {
            ...
            return (
              <div style={{padding:"0 16px",maxWidth:480,margin:"0 auto 8px"}}>
                <div style={{background:cardBg,borderRadius:14,padding:14,boxShadow:"0 2px 12px rgba(0,0,0,0.07)"}}>
                  ...
                  {jData.length>0 ? (
                    <svg width={W2} height={H2} style={{display:"block",margin:"0 auto",overflow:"visible"}}>
                      {[0,Math.round(jMax/2),Math.round(jMax)].map((v,i)=>(
                        <g key={i}>
                          <line x1={pL} y1={pY(v)} x2={W2-pR} y2={pY(v)} stroke={gridC} strokeWidth={1}/>
                          <text x={pL-4} y={pY(v)+4} textAnchor="end" fontSize={9} fill={tCol}>{v>=1000?(v/1000).toFixed(1)+"L":v}</text>
                        </g>
                      ))}
                      <text x={pL-2} y={pT-2} fontSize={9} fill={tCol}>ml</text>
                      {jData.map((d,i)=>{
                        const x=bX(i,jData.length),w=bW(jData.length),h=Math.max(1,gH2-(pY(d.val)-pT));
                        const hasMat=d.mat>0,hasComm=d.comm>0;
                        const matH=d.val>0?Math.round(h*(d.mat/d.val)):0;
                        const commH=h-matH;
                        const topColor=hasMat&&hasComm?(colorMat):hasMat?colorMat:hasComm?colorComm:colorSolo;
                        return (
                          <g key={i}>
                            {hasMat&&hasComm ? (<>
                              <rect x={x} y={pY(d.val)} width={w} height={matH} fill={colorMat} fillOpacity={0.8} rx={3}/>
                              <rect x={x} y={pY(d.val)+matH} width={w} height={Math.max(1,commH)} fill={colorComm} fillOpacity={0.8} rx={0}/>
                            </>) : (
                              <rect x={x} y={pY(d.val)} width={w} height={h} fill={topColor} fillOpacity={0.8} rx={3}/>
                            )}
                            <text x={x+w/2} y={H2-4} textAnchor="middle" fontSize={8} fill={tCol}>{d.label}</text>
                            {d.val>0&&<text x={x+w/2} y={pY(d.val)-3} textAnchor="middle" fontSize={8} fill={dark?"#f48fb1":"#c2185b"} fontWeight="bold">{fmtVal(Math.round(d.val))}</text>}
                          </g>
                        );
                      })}
                    </svg>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:100,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
          <div style={{background:cardBg,borderRadius:"24px 24px 0 0",padding:"28px 20px 40px",width:"100%",maxWidth:480,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 -8px 40px rgba(0,0,0,0.2)"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
              <button onClick={()=>setForm(f=>({...f,regurgi:Math.max(0,(parseInt(f.regurgi)||0)-1)}))}
                style={{width:36,height:36,borderRadius:"50%",border:`1.5px solid ${dark?"#555":"#ddd"}`,background:dark?"#2a2a3e":"#f5f5f5",color:textPrimary,fontSize:20,fontWeight:"bold",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
                −
              </button>
              <div style={{flex:1,textAlign:"center"}}>
                {(parseInt(form.regurgi)||0) === 0 ? (
                  <span style={{fontSize:14,color:dark?"#555":"#bbb"}}>Aucune</span>
                ) : (
                  <span style={{fontSize:22,fontWeight:"bold",color:dark?"#ffe082":"#f57f17"}}>
                    🤮 {form.regurgi} {parseInt(form.regurgi)===1?"régurgi":"régurgis"}
                  </span>
                )}
              </div>
              <button onClick={()=>setForm(f=>({...f,regurgi:(parseInt(f.regurgi)||0)+1}))}
                style={{width:36,height:36,borderRadius:"50%",border:`1.5px solid ${dark?"#555":"#ddd"}`,background:dark?"#2a2a3e":"#f5f5f5",color:textPrimary,fontSize:20,fontWeight:"bold",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
                +
              </button>
            </div>
...
// ── Sub-components ───────────────────────────────────────────────────────────
// ── CoucheCard ───────────────────────────────────────────────────────────────
function CoucheCard({ f, onEdit, onDelete, dark, cardBg, textPrimary, textSecondary }) {
  return (
...
// ── TireLaitTab ───────────────────────────────────────────────────────────────
function TireLaitTab({ tireLait, dark, cardBg, textPrimary, textSecondary, dynInputStyle, borderColor, setConfirmDelete, showToast, onFormOpen, onFormClose }) {
  const [showForm, setShowForm] = useState(false);
  const [editId, setEditId] = useState(null);
  ...
  // Year graph: Jan→Dec of year from selectedMonth
          ...
          <svg width={W} height={H} style={{display:"block",margin:"0 auto",overflow:"visible"}}>
            {[0,Math.round(maxVal/2),Math.round(maxVal)].map((v,i)=>(
              <g key={i}>
                ...
                <text x={padL-4} y={py(v)+4} textAnchor="end" fontSize={9} fill={textCol}>{v>=1000?(v/1000).toFixed(1)+"L":v}</text>
              </g>
            ))}
            <text x={padL-2} y={padT-2} fontSize={9} fill={textCol}>ml</text>
            {graphData.map((d,i)=>(
              <g key={i}>
                <rect x={pxBar(i,graphData.length)} y={py(d.val)} width={barW(graphData.length)} height={Math.max(1,gH-(py(d.val)-padT))} fill={dark?"#42a5f5":"#1565c0"} fillOpacity={0.75} rx={3}/>
                <text x={pxBar(i,graphData.length)+barW(graphData.length)/2} y={H-4} textAnchor="middle" fontSize={8} fill={textCol}>{d.label}</text>
                {d.val>0&&<text x={pxBar(i,graphData.length)+barW(graphData.length)/2} y={py(d.val)-3} textAnchor="middle" fontSize={8} fill={dark?"#90caf9":"#1565c0"} fontWeight="bold">{fmtV(Math.round(d.val))}</text>}
              </g>
            ))}
          </svg>
          );
...
function GrowthLineChart({ data, valueKey, color, unit, dark, whoTable, birthDate, showWho, birthPoidsG, birthHeureNaissance, weightUnit }) {
  ...
  const yLabels = [minV, minV+range/2, maxV].map(v => {
    ...
    return Math.round(v*10)/10; // cm, po
  });
  ...
  const whoLinePath = pIdx => {
    if (!whoPoints) return "";
    // Convert WHO kg/cm to display unit
    const factor = unit==="g" ? 1000 : unit==="lb" ? 2.205 : unit==="po" ? (1/2.54) : 1;
  ...
  function handleDotClick(e, d, i, val, prevVal) {
    ...
    if (whoPoints && whoTable) {
      ...
      if (ref) {
        ...
        // Exact percentile by linear interpolation between known percentile points
        ...
        return (
          <div onClick={()=>setTooltip(null)} style={{position:"absolute",top:top+"px",left:left+"px",transform:"translateY(-100%)",background:dark?"#2a2a3e":"white",border:`2px solid ${color}`,borderRadius:12,padding:"10px 14px",fontSize:12,color:dark?"#f5deb3":"#333",boxShadow:"0 4px 24px rgba(0,0,0,0.25)",zIndex:99999,width:bubbleW+"px",cursor:"pointer"}}>
            ...
            {pct && (
              <div style={{background:dark?"#1a1a2e":"#f5f5f5",borderRadius:8,padding:"5px 10px",marginBottom:4}}>
                ...
                <div style={{display:"flex",alignItems:"baseline",gap:6}}>
                  {exactPct!==null && (
                    <span style={{fontSize:18,fontWeight:"bold",color:exactPct<15?"#e53935":exactPct<50?"#ff9800":exactPct<85?"#4caf50":"#1565c0"}}>
                      {exactPct}e
                    </span>
                  )}
            ...
            {(() => {
              ...
              // Compute g/day from birth to this measurement date
              const [by,bmo,bd] = birthDate.split("-").map(Number);
              ...
              // val is in current display unit — convert back to grams
              ...
              return (
                <div style={{color:color2,fontSize:11,marginTop:2}}>
                  {parseFloat(gPerDay)>=0?"▲":"▼"} {Math.abs(gPerDay)}g/j depuis naissance
                </div>
              );
            })()}
          </div>
...
function GrowthTab({ growth, setGrowth, setShowGrowthForm, onEdit, profile, dark, cardBg, textPrimary, textSecondary, dynCardStyle, setConfirmDelete }) {
  ...
  const poidsParJour = (() => {
    ...
    const timeStr = profile.heureNaissance || "00:00";
    const [bh,bmi] = timeStr.split(":").map(Number);
    const birthDt = new Date(by, bmo-1, bd, bh, bmi);
    ...
    if (diffDays <= 0) return null;
    const diffG = parseFloat(latestPoids.poids) - birthPoids;
    return (diffG / diffDays).toFixed(1);
  })();
          ...
          <div style={{display:"flex", gap:8, flexWrap:"wrap"}}>
            ...
            {latestPoids && (
              <div style={{flex:"1 1 28%", background: dark?"#1a3020":"#e8f5e9", borderRadius:12, padding:"10px 10px", textAlign:"center", minWidth:90}}>
                ...
                {poidsParJour !== null && (
                  <div style={{fontSize:10, marginTop:2, color:parseFloat(poidsParJour)>=0?"#4caf50":"#e53935"}}>
                    {parseFloat(poidsParJour)>=0?"▲":"▼"} {Math.abs(poidsParJour)}g/j moy.
                  </div>
                )}
              </div>
            )}
...
function generateICS(appt, babyName) {
  ...
  // Sanitize: remove special chars AND normalize accented characters
  const sanitize = (s) => (s||"")
...
function SanteTab({ appointments, setAppointments, dark, cardBg, textPrimary, textSecondary, dynCardStyle, babyName, showToast, dynInputStyle, setConfirmDelete }) {
  ...
  function downloadICS(appt) {
    ...
    try {
      // Use Blob URL — works on iOS Safari and Android Chrome
